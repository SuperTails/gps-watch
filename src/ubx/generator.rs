...
pub trait SendablePacket {
    ...
    fn to_bytes(self) -> UbxGenerator<Self, Self::I> where Self: Sized {
        UbxGenerator (Some(GeneratorState::Sync1 {packet: self}))
    }
}

enum GeneratorState<T, I> where T: SendablePacket<I=I> {
    Sync1 {packet: T},
    Sync2 {packet: T},
    Class {packet: T},
...
impl<T, I> Iterator for UbxGenerator<T, I> where T: SendablePacket<I=I>, I: Iterator<Item = u8> {
    type Item = u8;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match core::mem::take(&mut self.0).unwrap() {
            ...
            Class {packet} => {
                ...
                self.0 = Some(Id {packet, checksum: UbxChecksum::new().next(val)});
                Some(val)
            },
            Id {packet, checksum} => {
                let val = packet.id();
                self.0 = Some(Len1 {packet, checksum: checksum.next(val)});
                Some(val)
            },
            Len1 {packet, checksum} => {
                let val = (packet.payload_len() as u16).to_le_bytes()[0];
                self.0 = Some(Len2 {packet, checksum: checksum.next(val)});
                Some(val)
            },
            Len2 {packet, checksum} => {
                let val = (packet.payload_len() as u16).to_le_bytes()[1];
                self.0 = Some(Payload {iter: packet.payload_bytes(), checksum: checksum.next(val)});
                Some(val)
            },
            Payload {mut iter, checksum} => {
                if let Some(val) = iter.next() {
                    self.0 = Some(Payload {iter, checksum: checksum.next(val)});
                    Some(val)
                } else {
